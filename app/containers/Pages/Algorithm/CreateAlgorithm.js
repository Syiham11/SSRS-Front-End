...
class CreateAlgorithm extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleSave = algoType => {
    ...
    if (algoType === 'derivative') {
      ...
      algo.algoName = nameD;
      algo.algoDescription = `${descD}  ${formulaD} : derivative ${numberOfDerivative} times by ${
        parametersD.name
      }`;
      algorithmsServices.saveAlgorithm(algo).then(result => {
        console.log(result.data);
    ...
    if (algoType === 'boolean') {
      const {
        booleanEq: {
          nameB, expression1, expression2, descB, typeB, signB
        },
        booleanEquations
      } = this.state;
      algo.algoName = nameB;
    ...
    this.openNotif('Algorithm saved');
  };
  ...
  handleChangeParaName = ev => {
    const { simpleEq } = this.state;
    const newSimpleEq = simpleEq;
    const testDt = newSimpleEq.parameters.find(
      p => p.id === Number.parseInt(ev.target.name, 10)
    );
    testDt.name = ev.target.value;
    newSimpleEq.parameters = [
      ...newSimpleEq.parameters.filter(
        p => p.id !== Number.parseInt(testDt.id, 10)
      ),
      testDt
    ].sort((a, b) => a.id - b.id);
    this.setState({ simpleEq: newSimpleEq });
  ...
  handleAddPara = () => {
    ...
    newSimpleEq.parameters = [
      ...newSimpleEq.parameters,
      { name: '', value: '', id: newSimpleEq.parameters.length + 1 }
    ];
    this.setState({ simpleEq: newSimpleEq });
  };
  ...
  handleChangeParaValueDerivative = ev => {
    ...
    const testDt = newSimpleEq.parametersDa.find(
      p => p.name === ev.target.name
    );
    console.log('value', ev.target.value);
    testDt.value = ev.target.value.toString();
    console.log(testDt);
    newSimpleEq.parametersDa = [
      ...newSimpleEq.parametersDa.filter(p => p.name !== testDt.name),
      testDt
    ];
  ...
  handleChangeParaValueBoolean = ev => {
    ...
    testDt.value = ev.target.value.toString();
    console.log(testDt);
    newSimpleEq.parametersB = [
      ...newSimpleEq.parametersB.filter(p => p.name !== testDt.name),
      testDt
    ];
  ...
  handleSimpleEqApply = der => {
    if (der === 'simple') {
      ...
      const {
        derivativeEq: { parametersDa, formulaDCheck }
      } = this.state;
      const val = {};
      ...
      this.setState({
        evaluateResult: simplify(formulaDCheck, val).toString()
      });
    } else if (der === 'integrate') {
      const {
        integrateEq: { formulaI, from, to }
      } = this.state;
      ...
      const {
        booleanEq: {
          expression1, expression2, typeB, parametersB
        }
      } = this.state;
      let x = '';
      const val = {};
  ...
  handleCheckFormula = () => {
    const { derivativeEq } = this.state;
    const x = nerdamer(
      `diff(${derivativeEq.formulaD}, ${derivativeEq.parametersD.name}, ${
        derivativeEq.numberOfDerivative
      })`
    );
  ...
  handleCheckFormulaBoolean = () => {
    ...
    const newBooleanEq = booleanEq;
    const y = nerdamer(
      `${newBooleanEq.expression1}+ ${newBooleanEq.expression2}`
    ).variables();
    const par = [];
    y.forEach(na => {
      par.push({ name: na, value: '' });
          ...
          <Grid item xs={12} sm={3}>
                ...
                <Tooltip
                  title={(
                    <Typography variant="body1">
                      Click to see all the Linear Equations already created
                    </Typography>
                  )}
                  arrow
                  enterDelay={200}
            ...
            {type === 1 ? (
              <div>
                ...
                <ListItem>
                  <Chip
                    label="Give values to test your Formula "
                ...
                <ListItem>
                  <Tooltip title="add new parameter" arrow>
          ...
          <Grid item xs={12} sm={3}>
                  ...
                  <Tooltip
                    title={(
                      <Typography variant="h5" component="p">
                        Click to see all the Derivative Equations already
                        created
                      </Typography>
                    )}
                    arrow
                ...
                <Tooltip
                  title={(
                    <Typography component="p">
                      Click To add New Derivative Equation
                    </Typography>
                  )}
                  arrow
                >
            ...
            {type === 2 ? (
              <div>
                ...
                <Tooltip title="click to check your Formula" arrow>
                  <IconButton
                    color="primary"
            ...
            <Box display="flex" component={ListItem}>
              ...
              <Box>
                <Tooltip
                  title={(
                    <Typography variant="body1">
                      Click To add New Integrate Equation
                    </Typography>
                  )}
                  arrow
                >
          ...
          <Grid item xs={12} sm={3}>
            <Box display="flex" component={ListItem}>
              ...
              <Box>
                <Tooltip
                  title={(
                    <Typography variant="body1">
                      Click To add New Boolean Equation
                    </Typography>
                  )}
                  arrow
                >
                  <IconButton
                    color="primary"
                    onClick={() => this.handleChangeType(4)}
                  >
                    <AddIcon />
                  </IconButton>
                </Tooltip>
            ...
            {type === 4 ? (
              <div>
                <ListItem>
                  <Button
                    onClick={() => this.handleChangeType(6)}
                    color="primary"
                  >
                    Text
                  </Button>
                ...
                <ListItem>
                  <FormControl fullWidth>
                    <InputLabel>Boolean Type</InputLabel>
                    <Select
                      name="typeB"
                      value={typeB}
                      onChange={event => this.handleChange(event, 'boolean')}
...
export default withStyles(styles)(CreateAlgorithm);
